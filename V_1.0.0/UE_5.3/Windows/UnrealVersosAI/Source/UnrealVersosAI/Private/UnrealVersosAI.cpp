...
{
    static UVersosBlueprintLibrary* Instance = nullptr;
    if(!Instance)
    {
        Instance = NewObject<UVersosBlueprintLibrary>();
        Instance->AddToRoot();
    }
    return Instance;
}

void UVersosBlueprintLibrary::PostImageToAPI(const FString& ImagePath,
...
FGridDimensions UVersosBlueprintLibrary::PlotGridOnImage(const FString& InputImagePath, const FString& OutputImagePath, const int CellSize)
{
...
FString UVersosBlueprintLibrary::Base64EncodeFile(const FString& Filename)
{
