...
{
    FRegexPattern BulletPattern(TEXT("^(?:[\\\"']?)((?:-|\\b[1-9][0-9]?[\\.-]))\\s"));
...
TArray<FString> AUVersosBlueprintLibrary::CleanUpBullets(const FString& JsonString, const int32 PaddingLength)
{
    ...
    {
        FString DataString = JsonObject->GetStringField("data");
...
double AUVersosBlueprintLibrary::CosineSimilarity(const TArray<double>& VectorA, const TArray<double>& VectorB)
{
    if (VectorA.Num() != 256 || VectorB.Num() != 256)
    {
        UE_LOG(LogTemp, Error, TEXT("Vectors must be of length 256"));
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("One of the vectors is zero-length"));
        return -1.0; // Indicating an error
    }
...
TArray<FText> AUVersosBlueprintLibrary::StringArrayToTextArray(const TArray<FString>& StringArray)
{
    TArray<FText> TextArray;
    TextArray.Reserve(StringArray.Num());

    for (const FString& Str : StringArray)
    {
        TextArray.Add(FText::FromString(Str));
    }

    return TextArray;
}

TArray<FText> AUVersosBlueprintLibrary::SliceArray(const TArray<FText>& InputArray, int32 Index, int32 Length)
{
    // Check if the requested slice is out of bounds
    if (Index < 0 || Length < 0 || Index + Length > InputArray.Num())
    {
        return TArray<FText>(); // Return an empty array if out of bounds
    }

    // Create the output array and reserve the needed size
    TArray<FText> SliceArray;
    SliceArray.Reserve(Length);

    // Copy the elements from the input array to the slice array
    for (int32 i = Index; i < Index + Length; ++i)
    {
        SliceArray.Add(InputArray[i]);
    }
...
FString ReadFileFromPluginResources()
{
    // Get the plugin's base directory
    FString PluginBaseDir = IPluginManager::Get().FindPlugin(TEXT("YourPluginName"))->GetBaseDir();
    
    // Construct the full path to the file in the Resources directory
    FString FilePath = FPaths::Combine(PluginBaseDir, TEXT("Resources/modelviewer.html"));
...
void AUVersosBlueprintLibrary::StartWebServer()
{
    WebServerInstance = NewObject<UWebServer>();
    WebServerInstance->AddToRoot();
}
